...
function Note({ options = false, setState, state, id }) {
  const dispatch = useDispatch();
  const [active, setActive] = useState(true);
  const [title, setTitle] = useState('This is title');
  const [content, setContent] = useState([{ desc: 'Drink water', status: true }]);
  ...
  function addOption() {
    const temp = [...contentTemp];
    temp.push({ desc: 'todo', status: true });
    setContentTemp(temp);
  }
    ...
    <div className={active ? 'note active' : 'note'}>
      {active ? (
        ...
        <>
          ...
          {content.map((el, i) => (
            <div className="checkbox" key={i}>
              <input type="checkbox" name={el.desc} id={el.desc} defaultChecked={el.status} />
